...
```{r}
L_ck = Lest(functionalOsun_ppp_jit, correction = "Ripley")
plot(L_ck, . -r ~ r, 
     ylab= "L(d)-r", xlab = "d(m)")
```

```{r}
L_ck.csr <- envelope(functionalOsun_ppp_jit, Lest, nsim = 39, rank = 1, glocal=TRUE)
```
...
```{r}
L_ck_non = Lest(nonfunctionalOsun_ppp_jit, correction = "Ripley")
plot(L_ck_non, . -r ~ r, 
     ylab= "L(d)-r", xlab = "d(m)")
```

```{r}
L_ck_non.csr <- envelope(nonfunctionalOsun_ppp_jit, Lest, nsim = 39, rank = 1, glocal=TRUE)
```

```{r}
plot(L_ck_non.csr, . - r ~ r, xlab="d", ylab="L(d)-r")
```
